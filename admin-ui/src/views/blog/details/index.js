...
const BlogDetails = () => {
  ...
  useEffect(() => {
    axiosInstance.get(`/get_post/${id}`).then(res => setData(res.data)).catch(err=>console.log(err))
  }, [])
          ...
          <div className='content-body'>
            {data !== null ? (
              <Row>
                <Col sm='12'>
                  <Card className='mb-3'>
                    {/* <CardImg src={data.image_Field} className='img-fluid' top /> */}
                    <CardImg src={`${baseURL}${data.image_Field}`} className='img-fluid' top />
                    <CardBody>
                      <CardTitle tag='h4'>{data.Post_Textfield}</CardTitle>
                      <div className='d-flex'>
